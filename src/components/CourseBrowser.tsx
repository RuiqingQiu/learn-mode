...
export default function CourseBrowser() {
  const { data, error } = useLoader(() => api.listCourses(), []);
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-10">
      ...
      {[...bySubject.entries()].map(([subject, list]) => {
        ...
        const sorted = [...list].sort((a, b) => Number(b.enrolled) - Number(a.enrolled));
        return (
            ...
            <div className="space-y-3">
              {sorted.map((c) => (
                ...
                >
                  ...
                  {c.enrolled && <ProgressRing done={c.completed} total={c.topics.length} />}
                </Link>
              ))}
            </div>
          </section>
